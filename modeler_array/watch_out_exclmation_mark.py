...
def main():
    ...
    while running:
        ...
        for obj_name, obj in NEW_STUFF.items():
            calculate_position(obj['object_class'], 0, 0, 0, -.03, 0, 0)
        for obj_name, obj in DICT.items():
            calculate_position(obj['object_class'], 0, 0, 0, -.03, 0, 0)
        ...
        for obj_name, obj in DICT.items():
            if obj['render']:
                ...
                if obj['move']:
                    calculate_position(obj_class, angle_x, angle_y, angle_z, pos_x, pos_y, pos_z)
        ...
        # Add new squares every 3 seconds
        if pygame.time.get_ticks() % 3000 < 50 and len([key for key in NEW_STUFF if key.startswith('square_')]) < 10:
            for _ in range(10):
                NEW_STUFF[f'square_{pygame.time.get_ticks()}'] = {
                    'object_class': Object('square'),
                    'render': True,
                    'move': False,
                    'collision': True,
                    'start_pos': (random.uniform(-3, 10), random.uniform(-10, 10), random.uniform(0, 7))
                }
            # Updates the start position of the objects\
            for obj_name, obj in NEW_STUFF.items():
                calculate_position(obj['object_class'], 0, 0, 0, *obj['start_pos'])
            DICT.update(NEW_STUFF)

        # Remove squares older than 10 seconds
        current_time = pygame.time.get_ticks()
        keys_to_remove = [key for key in NEW_STUFF if key.startswith('square_') and current_time - int(key.split('_')[1]) > 10000]
